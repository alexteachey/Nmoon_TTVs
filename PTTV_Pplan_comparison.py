...
try:
	...
	#### NOW WE CAN COMPUTE MOON PROBABILITIES FOR EACH BIN, GIVEN SOME PROBABILITIES OF FINDING MOONS
	#### THAT IS, WHAT IS P(moon | Pplan)?
	###### WHAT DISTRIBUTIONS DO WE WANT TO EXPLORE?
	1) uniform distribution of moons across Pplan
	2) loguniform distribution of moons across Pplan
	3) falling off like 1/P
	4) going up linearly with P

	#NOTE THAT PTTV IS NOWHERE IN THIS EXPRESSION, IT'S A 1-D DISTRIBUTION!
	#### THEREFORE, ASSIGN PROBABILITIES BASED ON PPlan alone!

	"""
	"""

	def histogrammer(data, logscale=False, nbins=20, autoshow=True):
		datamin, datamax = np.nanmin(data), np.nanmax(data)
		if logscale == True:
			histbins = np.logspace(np.log10(datamin), np.log10(datamax), nbins)
		else:
			histbins = np.linspace(datamin, datamax, nbins)

		n, bins, edges = plt.hist(data, bins=histbins, facecolor='DodgerBlue', edgecolor='k')
		if logscale == True:
			plt.xscale('log')
		if autoshow == True:
			plt.show()
	...
	"""
	def probabilities(xmin, xmax, nvals=100, probtype='uniform', spacing='linear', show_plot='y'):
		if spacing == 'linear':
			xvals = np.linspace(xmin,xmax,nvals)
		elif spacing == 'log':
			xvals = np.logspace(np.log10(xmin), np.log10(xmax), nvals)
		xwidths, xcenters = [], [] ### will be one fewer than xvals
		for nx,x in enumerate(xvals):
			try:
				xwidths.append(xvals[nx+1] - xvals[nx])
				xcenters.append(np.nanmean((xvals[nx+1], xvals[nx])))
			except:
				pass
		xwidths = 1/10**np.array(xwidths)
		nbins = len(xwidths)
		if probtype == 'uniform':
			heights = np.linspace(1,1,nbins)
		elif probtype == 'linear':
			heights = xcenters
		elif probtype == 'quadratic':
			heights = xcenters**2

		areas = xwidths * heights 
		normed_areas = areas / np.nansum(areas)

		if show_plot == 'y':
			fig, (ax1, ax2) = plt.subplots(2)
			### linear spacing
			ax1.plot(xcenters, normed_areas)
			ax1.set_ylabel('Probability [linear]')
			ax1.set_xscale('linear')
			ax2.plot(xcenters, normed_areas)
	...
	#### THIS IS ALL WRONG -- FIX TOMORROW.
	def dist_probs(xmin, xmax, nvals, xdist, ydist):
		...
		if (ydist == 'uniform'):
			yvals = np.linspace(1,1,nvals)

		elif (ydist == 'linear'):
			yvals = xvals 

		elif (ydist == 'quadratic'):
			yvals = xvals**2
		...
		for nx,x in enumerate(xvals):
			try:
				xwidth = xvals[nx+1] - xvals[nx]
				yheight = yvals[nx]
				bin_areas.append(xwidth*yheight)
				xcenters.append(np.nanmean((xvals[nx+1], xvals[nx])))
			except: ### when you get to the last one
				break
		bin_areas = np.array(bin_areas)
		area_under_curve = np.nansum(bin_areas)

		xcenters = np.array(xcenters)
		normed_bin_areas = bin_areas / area_under_curve

		return xcenters, normed_bin_areas	
